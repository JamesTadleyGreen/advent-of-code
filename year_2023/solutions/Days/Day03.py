def solution() -> int:
    ...
    for row, line in enumerate(INPUT):
        numbers = [x for x in line.split('.') if try_to_int(x)]
        for number in numbers:
            start, end = find_substring(row, number)
            gears = get_gears(row, start, end)
            for gear in gears:
                if gear not in gear_locations:
                    gear_locations[gear] = [number]
                else:
                    gear_locations[gear] += [number]
    return sum([int(v[0]) * int(v[1]) for v in gear_locations.values() if len(v) == 2])

def find_substring(row: int, number: str) -> tuple[int, int]:
    start = INPUT[row].find(number)
    return start, start + len(number)

def get_gears(row: int, start: int, end: int) -> list[tuple[int, int]]:
    coords = [(x, y) for x in range(start-1, end+1) for y in [row-1, row+1]] + [(start-1, row), (end+1, row)]
    coords = [c for c in coords if (c[0] in range(HEIGHT) and c[1] in range(WIDTH))]
    return [c for c in coords if INPUT[c[1]][c[0]] == '*']

def try_to_int(x: int) -> bool:
    try:
        int(x)
    except Exception:
        return False
...
INPUT = """.......855.442......190..................................969..........520.......59.............................................172..........
