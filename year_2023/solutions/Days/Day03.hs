...
day03 :: Solution
day03 = Solution input' parseInput part1 part2
...
part1 a =
  Just $
  snd $
  foldr (\s (i, v) -> (i + 1, v + checkLine a 0 0 "" i s)) (0, 0) (reverse a)

(.!!) :: [String] -> (Int, Int) -> Char
(.!!) s (i, j) = s !! j !! i

bounds :: [String] -> (Int, Int)
bounds s = (length (head s) - 1, length s - 1)

checkLine :: [String] -> Int -> Int -> [Char] -> Int -> String -> Int
checkLine a start end "" row [] = 0
checkLine a start end acc row [] =
  if hasSymbolNeighbour a start end row
    then (read (reverse acc) :: Int)
    else 0
checkLine a start end acc row (x:xs)
  | isDigit x = checkLine a start (end + 1) (x : acc) row xs
  | acc == "" = checkLine a (end + 1) (end + 1) "" row xs
  | otherwise =
    (if hasSymbolNeighbour a start (end - 1) row
       then (trace (show (reverse acc)) (read (reverse acc) :: Int))
       else 0) +
    checkLine a (end + 1) (end + 1) "" row xs

hasSymbolNeighbour :: [String] -> Int -> Int -> Int -> Bool
hasSymbolNeighbour a start end row =
